...
include/troop_table.h (what one troop costs, from the Soldier table),
gamedata/game_heroes.json + docs/heroes.md (the heroes: id, title, name, class, damage type) and gamedata/game_monsters.json (the map monsters: id, name, type, defence), and include/hunt_table.h (both, for the bot).
"""
import argparse
import collections
import json
...
def main():
    ...
    extract_soldiers(tables)
    extract_heroes(tables, fr, en)
    extract_monsters(tables, fr, en)
    write_hunt_header()
...
PLAYABLE_HERO_IDS = 200   # the heroes a player owns are ids 1-60 (+ a few special ones); the many rows above are enemy/NPC copies
HERO_ATTRIBUTE = {1: "Strength", 2: "Dexterity", 3: "Intelligence"}


def extract_heroes(tables, fr, en):
    """Heros.bin, 136-byte records (struct Hero in dump.cs): key u16, title u16 (the epithet the players use: "Snow Queen"), name u16 ("Alice"),
    default star u8, HeroType u8 at 0x07, ... AttackDamage u16 at 0x20, AbilityPower u16 at 0x22 (a mage has AbilityPower and no AttackDamage).
    Only the rows that have a title and an id below PLAYABLE_HERO_IDS are kept."""
    heroes = []
    for r in records(tables, "Heros", 136):
        key, title, name = struct.unpack("<HHH", r[:6])
        star, htype = r[6], r[7]
        strength, dexterity, intelligence = struct.unpack("<HHH", r[0x18:0x1e])
        hp, damage, power, armor, resist = struct.unpack("<HHHHH", r[0x1e:0x28])
        if not en(title) or key >= PLAYABLE_HERO_IDS:
            continue
        heroes.append(dict(id=key, title_en=en(title), title_fr=fr(title), name_en=en(name), name_fr=fr(name), default_star=star, hero_type=htype,
                           attribute=HERO_ATTRIBUTE.get(htype, "?"), attack_damage=damage, ability_power=power, armor=armor, magic_resist=resist,
                           damage="magic" if power > damage else "physical" if damage > power else "none"))
    heroes.sort(key=lambda h: h["id"])
    out = {"source": "Heros.bin of the game's Loading/Table.unity3d (struct Hero); names from its string tables. hero_type 1 = Strength, 2 = Dexterity, "
                     "3 = Intelligence (the class); damage = which of AttackDamage / AbilityPower is the hero's.", "heroes": heroes}
    with open(os.path.join(ROOT, "gamedata", "game_heroes.json"), "w", encoding="utf-8") as f:
        json.dump(out, f, ensure_ascii=False, separators=(",", ":"))
    lines = ["# Heroes: title <-> id",
             "",
             ...
             "|---:|---|---|---|---|---|"]
    for h in heroes:
        lines.append("| %d | %s | %s | %s | %s | %s |" % (h["id"], h["title_en"], h["title_fr"] or "", h["name_en"] or "", h["attribute"], h["damage"]))
    with open(os.path.join(ROOT, "docs", "heroes.md"), "w", encoding="utf-8", newline="\n") as f:
        f.write("\n".join(lines) + "\n")
    print(f"gamedata/game_heroes.json + docs/heroes.md: {len(heroes)} heroes")


MONSTER_KIND_MAP = 10   # byte 4 of a Monster.bin row = the kind of the map record (10 = the monsters of the map, seen in a capture)


def extract_monsters(tables, fr, en):
    """Monster.bin, 57-byte records: key u16 (the key a map monster carries and the attack sends), kind u8 at 4, name u16 at 5, attack type u16 at 11
    ("Physical" / "Magic" / "Physical and Magic" / "Direct DMG"), defence u16 at 13 ("High PDEF" / "High MDEF" / "Physical and Magic"). Read from the
    Gorzilla, which the owner's game shows as "Physical - High PDEF"."""
    monsters = []
    for r in records(tables, "Monster", 57):
        key = struct.unpack("<H", r[:2])[0]
        kind = r[4]
        name, attack, defence = struct.unpack("<HHH", bytes(r[5:7]) + bytes(r[11:13]) + bytes(r[13:15]))
        if not en(name):
            continue
        monsters.append(dict(id=key, kind=kind, name_en=en(name), name_fr=fr(name), attack_en=en(attack), attack_fr=fr(attack),
                             defence_en=en(defence), defence_fr=fr(defence)))
    out = {"source": "Monster.bin of the game's Loading/Table.unity3d; kind, name, attack type and defence read from its 57-byte rows.", "monsters": monsters}
    with open(os.path.join(ROOT, "gamedata", "game_monsters.json"), "w", encoding="utf-8") as f:
        json.dump(out, f, ensure_ascii=False, separators=(",", ":"))
    print(f"gamedata/game_monsters.json: {len(monsters)} monsters")


def write_hunt_header():
    """include/hunt_table.h from the two JSON files just written: which damage each hero fights with, and what each map monster is weak to."""
    heroes = json.load(open(os.path.join(ROOT, "gamedata", "game_heroes.json"), encoding="utf-8"))["heroes"]
    monsters = json.load(open(os.path.join(ROOT, "gamedata", "game_monsters.json"), encoding="utf-8"))["monsters"]
    damage = {"none": 0, "physical": 1, "magic": 2}
    weak = {"High PDEF": 1, "High MDEF": 2}          # "High PDEF" = strong against physical, so weak to magic; the balanced one has no weakness
    lines = [
    ...
    for h in heroes:
        lines.append("\t{ %d, %d }," % (h["id"], damage[h["damage"]]))
    lines += ["};", "",
    ...
    for m in monsters:
        lines.append("\t{ %d, %d, %d, %s, %s }," % (m["id"], m["kind"], weak.get(m["defence_en"], 0), c_str(m["name_fr"] or m["name_en"]), c_str(m["name_en"])))
    lines += ["};", "", "#endif"]
    with open(os.path.join(ROOT, "include", "hunt_table.h"), "w", encoding="utf-8", newline="\n") as f:
        f.write("\n".join(lines) + "\n")
    print(f"include/hunt_table.h: {len(heroes)} heroes, {len(monsters)} monsters")


def extract_soldiers(tables):
    """What one troop costs (Soldier table, SoldierData: 42-byte records). Kind 0 rows are the troops: SoldierKind 0-3 =
    infantry, ranged, cavalry, siege (the bot's TroopKind), Tier 1-4. The T5 rows cost nothing here (T5 is made from T4)."""
    cost = {}
    for r in records(tables, "Soldier", 42):
        (_key, kind, _name, _icon, _model, _cap, skind, tier, _atk, _df, _hp, _spd, _traffic, _str, _sal, _rad, _skill,
         food, stone, wood, iron, money, seconds, _sci) = struct.unpack("<HBHHHHBBHHHHBBBHHHHHHHHH", r)
        if kind == 0 and 1 <= tier <= 4 and skind <= 3:
            cost[(skind, tier - 1)] = (food, stone, wood, iron, money, seconds)
    if len(cost) != 16:
        sys.exit(f"Soldier: {len(cost)} troop rows instead of 16 (4 kinds x T1-T4), the layout changed")
    lines = [
    ...
    for k in range(4):
        lines.append("\t{")
        for t in range(4):
            lines.append("\t\t{ %s }," % ", ".join(str(x) for x in cost[(k, t)]))
        lines.append("\t},")
    lines += ["};", "", "#endif"]
    path = os.path.join(ROOT, "include", "troop_table.h")
    with open(path, "w", encoding="utf-8", newline="\n") as f:
        f.write("\n".join(lines) + "\n")
    print(f"{path}: 16 troop rows")
