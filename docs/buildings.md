...
| Help | 2852 `_MSG_RESP_ALLIANCE_SOMEBODY_NEEDHELP` (client), 2853 (answer) | The client asks the alliance for help by itself after every start: payload `u32 seq, u8 1` (the research one sent 0: most likely the kind). The byte is **1 for the captured farm, which ran in queue 0** (0 for a research). Sent as 1 for a farm in queue 1 the server answered **status 2** (`02 01 5d 87 02 00`, `02 01 22 00 01 00`) and nobody got a request: so the bot now sends `queue + 1` - *inferred*. No building in the request: the server helps the one in that queue. Accepted answer (2853, dispatched from the enum's `_MSG_RESP_ALLIANCE_HELP`): `00 <the byte> <build_id> <level> 1e`. `RequestBuildHelp()`, `RecvBuildHelpAnswer()`. **Send the number 2852, not the enum constant**: `_MSG_RESP_ALLIANCE_SOMEBODY_NEEDHELP` in `packet_enum.h` evaluates to 2854 (the enum drifts by 2 there), which the server does not know: it closed the connection a few seconds after. |
...
| **Cancel** (request) | 2006 `_MSG_REQUEST_BUILDCANCEL` | `u32 seq, u8 0` (5 bytes): the queue number, 0 = the base queue (the farm it cancelled ran in queue 0 while the Mana Lode ran in queue 1). Answer 2007, 23 bytes: the five stocks after the refund (u32 each) and **the queue that was cancelled (u8, offset 20)**, then 2 bytes (live: `... 54 c6 43 01 | 01 00 00` for a farm in queue 1; the bot checks it is the queue it asked for). `RequestBuildCancel()`, used only by `$askhelp`. |
